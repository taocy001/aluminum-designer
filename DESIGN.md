...
- 单元测试（vitest，`src/__tests__`）：接头修剪、柜体下料长度、重叠规则、屏幕空间拾取/轴向解析。
- 端到端（Playwright 无头 Chromium + SwiftShader，`e2e/`）：41 个用例覆盖绘制全部路径、五种规格、选择/框选、拖拽（吸附/阻挡/竖直/成组/撤销）、键盘编辑、属性面板、BOM/CSV/工程文件、清空二次确认、语言切换、持久化、14 种连接件放置与删除、完整柜体搭建。
