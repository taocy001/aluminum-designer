...
11. [v3.0 从"画得出"到"交得出"](#11-v30-从画得出到交得出)
...
### 10.20 中心线建模 vs 贴面装配（2026-09-22，依据用户指出的四个问题）

用户看完示例柜子提了四条：横着的 2040 和角落的 4040 用角码拼不起来、2020 和 2040 的侧面拼接同理、角码乱放、缺隔层。全部属实，而且**前三条是同一个根因**。

**根因：模型按中心线建，装配按面来做。**
型材画在中心线网格上（人就是这么想的），但角码是一块贴在两根型材**外表面**上的弯板。
它能贴平，当且仅当那两个面**共面**。实测当时的示例：112 个角码里 **106 个埋在型材肚子里**。

**判据从"规格有共同边"升级为两个条件同时成立**（`specCompat.ts`）
1. **截面有共同边**——否则角码没有可对齐的边、孔位也对不上（2020 挂 4040 无论怎么推都不行）
2. **实际对接面共面**——角码的背面垂直于两根的轴向，只能是二者叉积方向；每根在该方向上有两个面（中心线 ± 半截面），必须有一对重合

第二条解释了为什么"2040 接 4040 可以"要看**怎么放**：横梁对着立柱中心线走，面差 10mm，贴不平；横梁贴着立柱外表面走，才共面。

**创建时对齐**（`faceAlign.faceAlignOnCreate`，在 `tryAddProfile` 里调用）
新画的型材落下去时，按它接触到的已有构件把自己横向平移，使面共面。已有构件不动，所以第一根定平面、后面的跟随，**不会级联**。

**为什么不做"一键修复整图"**：试过了，不收敛。把横梁挪去贴合立柱，就把它从站在其上的分隔柱底下抽走；58 → 50 → 52 → 50 来回震荡，还撞出 6 处干涉。这个方案没有发出去。

**只纠正截面台阶，不纠正手抖**：`shiftForJoint` 要求两根中心线在该方向上本来就重合，才认为面差来自截面尺寸差异。否则那是"点歪了 5mm"，属于定位错误——替用户把它抹平，等于把一次手滑变成权威坐标，而且因为幅度更小还会压过真正该做的 10mm 修正。工具如实报告，由人去改。

**截面朝向成为一等属性**（`rollProfile` / `sectionFacing`）：2040 立着放和躺着放是两种构件，承载方向和可安装面都不同。属性面板显示长边朝向，一键转 90°。此前朝向由 `setFromUnitVectors` 随手决定，既不显示也不能改。

**角码放到真实位置**（`autoConnect`）：落在 `flushFace` 算出的共面上，而不是中心线端点。示例里从「106 埋在内部 / 0 贴面」变成「8 / 116」——剩下 8 个是同一接头上的第二块角码沿轴向退让后压进邻件，属于建模简化。

**示例重做**：L 形那套的转角在脚本里反复震荡（两个接头要求相反的 5mm），**删掉**。一套正确的示例胜过两套其中一套有已知坏接头。保留一字型厨房：70 根、124 个角码、10 块板（含 4 块隔板）、0 干涉、0 处贴不平。

### 10.21 外侧齐平、转角贯通方式、连接件反馈与缩放（2026-09-22）

**1. 贴面时优先对齐外侧面**（`faceAlign.shiftForJoint`）
伙伴的两个面都够得着，平移量还一样大，所以大小决定不了选哪个。改成取**更外侧**那个：框架是要外部齐平的，吊柜下方的进深横撑应当与柜体**下边沿**齐平，而不是上边沿。

**2. 转角贯通方式可切换**（`jointUtils.ThroughRule`，默认 `rails`）
只有转角需要这条规则——端头落在中段时答案是显然的。两种做法现实中都天天在用，而且**不等价**：横梁搭在立柱顶上，载荷顺着立柱直接往下传（受压）；立柱贯通则横梁只靠螺栓吊着。默认改为横梁贯通，侧栏可切。改这条会改变图里每一个修剪，所以它进了 `analyzeFrame` 的缓存键。原有 6 条单元测试描述的是旧规则，各自显式声明自己测哪一种，而不是依赖默认值。

**3. 连接件其实能选中，但没有任何悬停反馈**
悬停只跟踪型材（`hoverProfileId`），鼠标移到角码上什么都不亮，于是看起来像选不中。新增 `hoverPartId` 覆盖所有类型，连接件与板件都会高亮，光标也随之变成抓取态。

**4. 缩放按钮**：工具栏 − / +，沿视线推拉相机并保持注视点，限制与 OrbitControls 一致。给触控板用户和不想学手势的人。

**实建时工具当场揪出的设计错误**：端头进深横撑用 2040 时，"外侧齐平"会把它推到 4040 立柱**外面**（x=-10），再被贯通横梁延伸撞上，3 处干涉。改用 4040（与立柱同截面）即天然共面、无需平移。这正是这套检查该起的作用。

### 10.12 测试

- 单元测试（vitest，`src/__tests__`，63 个）：接头修剪、容错接触、共轴接续、干涉检查、柜体下料长度、重叠规则、屏幕空间拾取/轴向解析。
- 端到端（Playwright 无头 Chromium + SwiftShader，`e2e/`）：252 个用例覆盖手持模型（装填/放下/Esc 链/三键统一/端面两义性/手持时仍可选中拖动与用 Gizmo）、贴面装配（创建时对齐、优先外侧、共同边 + 共面双条件、截面朝向、角码落在共面上）、转角贯通方式、缩放按钮、连接件与板件的悬停反馈、接头可装配性（共同边/跨系列两级）、一键连接（角码/端盖/贴面件三类落位）、总尺寸标注、标准快捷键（Ctrl+A、双击取子装配、空格全局盘、Shift+F 全屏）、工作平面与落点提示、Tab 重叠拾取、终点坐标编辑、板件贯通件告警与嵌入模式、板件（按选区生成柜门/隔板、手性校验、尺寸与材质编辑、面拾取与拖动、锁定删除、下料清单归并、持久化）、空格快捷盘（开合、贴边不出窗、各项动作）、镜像（整体中心面、长度与跨度、单条历史）、阵列（间距与份数、落地钳制、解锁副本、单条历史）、旋转支点（中心/起点/终点与 Gizmo 跟随）、手势中途精确数值（移动距离、拉伸长度、单条历史）、锁定（拖动/删除/方向键/旋转/手柄全挡住，仍可作吸附参照）、合一控件（轴向移动、90 度旋转、让位规则）、绘制态拖动型材、连接件朝向与紧固件推导、 90 度旋转按钮、端面箭头、吸附反馈、画布内旋转/拉伸/绘制态旋转视角/侧栏折叠/对齐吸附、任意轴旋转与干涉告警、交互反馈批次（拖拽平面、转视角保选、拒绝反馈、拾取容差与遮挡、画布外取消、右键语义、光标状态）、手搓柜子（三种搭法，点击带 ±3px 随机偏移、点在型材表面）、端面/侧面拾取、绘制全部路径、五种规格、选择/框选、拖拽（吸附/阻挡/竖直/成组/撤销）、键盘编辑、属性面板、BOM/CSV/工程文件、清空二次确认、语言切换、持久化、14 种连接件放置与删除、完整柜体搭建。
...
> 这一章记录 2026-09-22 起的一轮，主题是：图能画出来之后，**怎么保证图是对的、改得动、拿得走**。
> 每一条后面都附了它是怎么被发现的——大部分不是想出来的，是画八组柜子时撞出来的。

### 11.1 拾取必须和画面一致

**问题**：抽样 3707 个点比对"渲染器在这个像素上画的是谁"和"点击会选中谁"，一致率只有 36–83%。
根因是每种零件各有一套偏置（连接件往前提、板件放宽、构件按当前开合），这些偏置互相打架，
最后谁赢跟深度顺序无关。

**规则**（两条，有先后）：
1. **画面上画的是谁，点中的就是谁。** 对真实 mesh 做一次射线检测（`frontmost.ts`），
   命中的那个**直接提到候选列表最前**。这条不讲道理，也不需要讲道理。
2. 都没有正对命中时，才轮到屏幕距离和深度决胜负。

修完 3704/3707。剩下 3 个是抗锯齿边缘像素，不修。

**代价要说清楚**：门关上就挡住了框架，这时点击选中的是门，不是它后面的立柱——这是规则 1 的
直接后果，不是 bug。所以补了"收起柜门与抽屉"的开关，而不是去调偏置把规则 1 弄坏。

**注意**：drei 的粗线（`Line`）不设 `raycaster.camera` 就永远检测不到。

### 11.2 选得中不等于拖得动

用户反馈"选中背板或者抽屉挪不动"。我此前只审过**拾取**，没审过**拖动**——
`partById`、`item` 查找、`updateParts`、`commitTransform` 四个地方都漏了 fittings。
补完之后又补了 `nudgeSelected` / `rotateSelected`。

教训进了流程：**每加一种可选中的东西，就要问一遍"它能拖吗、能微调吗、能旋转吗、能撤销吗"**，
并且要有一份"拖动审计"脚本逐类采样，而不是只审拾取。
当前覆盖：型材 7/7、板件 7/7、构件 11/11、连接件 0/1（连接件按设计跟随型材，不单独拖）。

### 11.3 侧栏改数字要立刻看见

方向键调尺寸时图上不动，得敲回车或点别处才更新。原因是 `NumField` 把方向键交给浏览器的
number input 默认行为，只在 blur/Enter 时 `onCommit`。

改法：`NumField` 自己处理方向键（立即 `onCommit`），另给一个 `onLive` 走打字预览。
预览有个坑——store 里已经是新值了，`onCommit` 比较时看不到变化，撤销记录就没了；
所以预览开始时用 `beginLiveEdit()` 单独打一次快照。

### 11.4 门的动画要绕着铰链转

开关门时"铰链那一侧跑远了"。原因是把**位置**和**旋转**分别做缓动：
门的位置正是"让铰链边不动"所需要的那个偏移量，两者分开插值，中间帧两个条件都不满足。

改成只缓动**一个数**——开合度 `open`（0…1），每帧由它算出完整的 `openTransform`。
铰链边在任何一帧都不动，因为它在任何一个 `open` 值下都不动。

### 11.5 标注

- **W/H/T 标在框内**：每个数字画在它所量的那条边的**内侧**，而不是甩到外面。
- **不再一律画在最上层**：被挡住的零件的标注也被挡住，否则画面全是浮在空中的数字。
- **贴面的标注会被切一半**：sprite 是面向相机的四边形，会横穿它所贴的那个面。
  每帧朝相机方向倾 40 mm 即可。

### 11.6 一键修接头（`repairJoints.ts`）

工具早就能说"这 21 个接头没处上螺栓"，然后一点忙都不帮。八组柜子撞了 32 次。

两个动作，有先后：**先转**（2040 转 90° 不移动、不改长，代价为零），**再挪**（只挪截面小的那根，
±5…±40 逐步试）。每一步都打分才保留。

**踩过的两个坑**：
- 全图打分 15 秒，按钮里放不下。改成只算**这根构件的邻域**——移动一根只可能改变它参与的接头和碰撞，
  所以邻域打分不是近似，是同一个答案。15 s → 3.2 s。
- **打分会被钻空子**：接头装不上得分差，而把构件推到够不着邻居，接头就不存在了，得分完美。
  修复真的把一根 4040 立柱挪出柱线 30 mm，然后报告"全部接头正常"。
  所以分数里必须带上**还连着几个接头**，丢接头的一步一律退回。

### 11.7 工作平面是一句话，不是一个默认值

把工作平面抬到 440 画隔板横撑，结果横撑画到了两米外的另一组柜子里——
屏幕上 18 px 的吸附半径，在那个视距下是 50 多毫米，正好够上别人家的柱顶。

**抬起工作平面 = 明确声明"我在这个高度上作业"**，所以抬起时：
- 不在这个面上的端点，不管离光标多近，都不参与吸附；
- 点在立柱上，取它**在这个面上**的那个点，而不是丢掉这一击。

地面（planeY=0）不算声明，行为不变。

### 11.8 挠度（`deflection.ts`）

"装得上"和"看起来是直的"是两个问题，工具只答了第一个。

- **截面属性用查表值，不用画出来的轮廓**。这里画的轮廓是实心减槽口，没有中孔没有腹板，
  惯性矩会大出一半，一个偏乐观的下沉值比没有还糟。
- **跨距是支点之间的距离，不是构件长度**。一根横梁压在它穿过的每一根立柱上。
  一开始按端到端算，把厨房吊柜那根 2700 的横梁报成下沉 90 mm，
  而它真正会弯的是留给油烟机的那 900。
- 悬挑单独算，并和跨中比"谁沉得多"，不是比谁更长。
- 自重计入。跨距/下沉 < 1/200 变琥珀色——那大约就是看得出弯的临界点。

**它当场揪出了我自己画的两处**：书桌上方 1400 宽的三层书架（25 kg 下沉 29 mm）——加一根中柱；
电视柜中间 1200 的开档（下沉 18.5 mm）——中间要放抽屉不能加柱，改用 2040 横梁。

### 11.9 别的

- **起步模板**（`templates.ts`）：工作台、置物架、单门柜、机柜、桌腿框架。
  第一版工作台和桌子分别带着 16 和 8 个装不上的接头出厂——2040 摆在 4040 的中线上。
  加了 `perimeter()` 按 `(postW - railW) / 2` 让横梁与立柱外表面齐平。
- **分享链接**（`shareLink.ts`）：按列打包 + `deflate-raw` + base64url，放在 URL 片段里，不经服务器。
  第一版的测试是假的——同一个浏览器里开两个页面共享 storage，什么都没证明。改成两个独立 context。
- **测量**（`measure.ts`）、**手势**（`Gestures.tsx`：捏合缩放、双指轻点撤销、长按菜单，
  避开 macOS 占用的三指四指和浏览器的双指横扫）。
- **柜门只能向外开**（`fittingOps.openingOutward`）：例子文件里的吊柜门朝里开。
  规则本身也脆——`carcaseAround` 原先按选中件自身高度向上扩，把地柜的顶横梁也算进来，
  于是吊柜被判成 640 深而不是 370。

### 11.10 八组柜子（验收用例）

按 100 ㎡ 两室两厅画了 8 组独立柜体，**在有头浏览器里用点击画**，不是写进 store 的：
厨房地柜 2700、厨房吊柜（灶上留油烟机）、冰箱+蒸烤箱高柜、玄关鞋柜、客厅电视柜、
主卧衣柜、次卧书桌+书架、阳台洗衣柜。

最终：228 根型材、324 个角码、8 块板、20 个构件，
**干涉 0、装不上的接头 0、角码落位错误 0、控制台报错 0、脚本层摩擦 0**。

这个用例揪出来的东西，比前面所有章节加起来还多。几条值得记住的：
- 横梁不能从立柱里穿过去——长边要**一档一档画**。
- 手画的线会在吸附半径内漂，画完要**按"起点—终点"把数字敲回去**；
  但敲完会把工具自动做的贴面偏移也抹掉，所以**敲完要再点一次"修复接头"**。
- **先做结构，再挂门**。门一挂上，它后面的那一档就点不到了——真装也是这个顺序。
