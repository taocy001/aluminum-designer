...
![L 形橱柜示例](examples/kitchen-l-5000x3000.png)

上图是用这个工具搭的一套 L 形整体橱柜：5040 × 3040 × 920，55 根型材、48 个角码、6 块板，
下料总长 66.88 m。工程文件和物料清单都在 [`examples/`](examples/) 里，可以直接导入打开。

## 它替你算的事

- **接头修剪**：立柱贯通、横梁切入柱面，中心线长度和**下料长度**是两个数，清单给的是后者
- **干涉检查**：型材穿模时标红并指出位置，但不阻止你继续画——先摆对，再调整
- **接头可装配性**：端面对接的两根必须有一条相等的边，否则角码无处可依。2020 挂 4040 会被标出来
...
| 在半空起画 | 挂靠已有型材的端点/表面；或把工具栏的**工作面**抬到目标高度（选中立柱后点 ↥ 即取其顶面） |
...
- **工程文件 JSON**：可再导入，版本化（当前 v2，旧文件自动兼容）
