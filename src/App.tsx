...
import { Languages, ShieldAlert, Activity, Home } from 'lucide-react'

function App() {
  const { selectProfile, profiles } = useStore()
  const { language, setLanguage, isDrawing, startPoint, currentPoint, viewMode, triggerCameraReset } = useToolStore()
      ...
      <header className="h-14 bg-slate-800 border-b border-white/10 flex items-center px-6 text-white shadow-2xl z-20">
        ...
        <div className="ml-auto flex items-center gap-4">
          <div className="hidden lg:flex items-center gap-4 px-4 py-1.5 bg-black/40 border border-white/10 rounded-full text-[10px] font-mono">
            ...
            <div className="text-blue-400">COUNT: {profiles.length}</div>
            <div className="w-[1px] h-3 bg-white/10"></div>
      ...
      <div className="flex-grow flex overflow-hidden">
        ...
        <main className="flex-grow relative" onClick={() => selectProfile(null)}>
          ...
          <div className="absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-1 bg-slate-900/90 backdrop-blur-md border border-white/10 rounded-xl p-1 shadow-2xl z-10">
            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold pointer-events-none ${
              viewMode === 'draw' ? 'text-blue-400' : 'text-slate-500'
            }`}>
              {viewMode === 'draw'
                ? (language === 'zh' ? '✏️ 绘制中' : '✏️ Drawing')
          ...
          {/* TELEMETRY OVERLAY */}
          <div className="absolute top-16 left-6 pointer-events-none space-y-2">
            <div className="bg-black/60 backdrop-blur-md p-3 rounded-lg border border-white/10 text-[9px] text-slate-300 font-mono shadow-xl">
              ...
              <p>MODE: {viewMode.toUpperCase()}</p>
              <p>DRAWING: {isDrawing ? 'ACTIVE' : 'READY'}</p>
              <p>START_PT: {startPoint ? `[${startPoint.x.toFixed(0)}, ${startPoint.y.toFixed(0)}, ${startPoint.z.toFixed(0)}]` : 'NONE'}</p>
